...
pub fn build_bbox_cache(command: RoadsBboxCommand) -> Result<(), String> {
    ...
    println!(
        "Scanning planet for roads in bbox [{:.4},{:.4}] x [{:.4},{:.4}]",
        bounds.min_lat, bounds.max_lat, bounds.min_lon, bounds.max_lon
    );

    let candidate_nodes = collect_candidate_nodes(&command.planet_path, expanded)?;
    println!("Collected {} candidate nodes", candidate_nodes.len());

    let roads_by_cell = collect_roads_by_cell(&command.planet_path, bounds, &candidate_nodes)?;
