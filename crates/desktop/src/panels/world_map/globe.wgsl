// ── Uniform struct ──────────────────────────────────────────────────────────
//
// This struct MUST stay byte-for-byte identical to `GlobeUniforms` in
// `globe_pass.rs`. See the offset table and the `size_of` assertion there.
// Any field added/removed/reordered must be mirrored in both places.
struct Uniforms {
