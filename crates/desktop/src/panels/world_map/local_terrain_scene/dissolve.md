...
## Purpose
Draws the terrain loading grid and local scene frame. Every tile's 100×100 cells
...
- `draw_tile_pulse_grid`: projects each pending tile footprint and removes
  exactly floor(progress × 10,000) cells. Missing progress is zero; ready/no-source
  buckets are suppressed. Time only varies brightness, never completion. Both
  main cells and chromatic fringes use 75% of their previous alpha. Entirely
  offscreen tile footprints are skipped before cell generation.
- `cell_ranks`: one cached shuffled permutation with a stable per-tile cyclic
  rank offset, avoiding repeated sorting/allocation for the denser grid and
  thresholds which can remove more/fewer cells than the reported fraction.
...
## Validation
Existing regressions use the 10,000-cell counts and check monotonically disappearing cell sets,
