...
### `paint`
- **Does**: Draws the globe backdrop, cached global coastlines, 3D wireframe globe, low-detail global contour context, markers, links, and HUD legend
...
- **Does**: Handle perspective globe projection, low-zoom raster-backed surface shading, path splitting by visible hemisphere, backdrop/HUD rendering, and overlay styling
- **Interacts with**: `egui::Painter`

### `draw_global_coastlines`
- **Does**: Draws a cached coarse `0m` coastline layer over the low-zoom globe using a zoom-banded simplified line set
- **Interacts with**: `contour_asset.rs`, `draw_geo_path`
- **Rationale**: Gives the analyst a recognizable world outline even when the underlying relief shading is intentionally subtle
...
- Mock geography has been removed entirely; if the runtime rasters are unavailable, the globe stays visually sparse instead of inventing coastlines.
- The horizontal screen projection now uses the corrected handedness, so focused longitudes and drag interaction no longer feel mirrored left-to-right.
