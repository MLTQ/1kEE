...
- **Interacts with**: `AppModel` in `model.rs`, panel renderers in `panels/`, `factal_stream.rs`, shutdown cleanup in `panels/world_map/srtm_focus_cache.rs`
...
- **Does**: Cancels active GDAL terrain-cache subprocesses and suppresses new Factal poll scheduling when the native app is tearing down
- **Interacts with**: `shutdown` in `factal_stream.rs`, `terminate_active_gdal_jobs` in `panels/world_map/srtm_focus_cache.rs`
