...
- Input: openly published camera metadata and feed URLs (registries + provider scrapers),
  including an explicit opt-in, paced Project Eyes On-compatible Insecam directory adapter.
...
- Progress: the background registry reports directory pages, candidates,
  cache reuse, geolocated cameras, and reachable feeds to a non-blocking
  top-bar indicator.
